...
BATCH_SIZE = 5           # النظام الهجين: 5 أعضاء بالدفعة لتجنب فشل الطلب
...
# [============== دالة العمل في الخلفية (النظام الهجين الذكي) ==============]
        ...
        try:
            ...
            for i in range(0, len(valid_users), BATCH_SIZE):
                ...
                try:
                    ...
                    # المستشعر الذكي لقراءة وقت الحظر
                    ...
                    LIVE_LOGS[phone].append('<span style="color:#f00;">⚠️ ضربة أمنية: تيليجرام حظر الحساب مؤقتاً بالكامل (Flood).</span>')
                    break
                except UserPrivacyRestrictedError:
                    LIVE_LOGS[phone].append('<span style="color:#aa0;">⏭️ بعض الأعضاء في الدفعة لديهم خصوصية تمنع إضافتهم. تم التخطي.</span>')
                except Exception as e:
                    # في حال وجود خطأ عام، يتم تخطي الدفعة وتجربة الدفعة التالية
                    LIVE_LOGS[phone].append(f'<span style="color:#f00;">⚠️ خطأ في الدفعة، جاري تجاوزها...</span>')
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', sans-serif; }
        ...
        .badge { background: rgba(59, 130, 246, 0.1); color: #60a5fa; padding: 5px 10px; border-radius: 20px; font-size: 12px; display: inline-block; margin-bottom: 15px; border: 1px solid rgba(59, 130, 246, 0.2); }
...
<body>
    <div class="container">
        <div class="badge">🚀 النظام الهجين (دفعات + قراءة ذكية)</div>
        <h2>إعدادات النقل</h2>
        ...
        <form method="POST" action="/submit_main">
            ...
            <button type="submit">بدء النظام الذكي 🚀</button>
        </form>
    </div>
</body>
</html>
...
<head>
    <meta charset="UTF-8">
    <title>البث المباشر (النظام الذكي)</title>
    <style>
...
<body>
    <h2>⚡ البث المباشر (النظام الهجين) ⚡</h2>
    <p>يتم إضافة الأعضاء بدفعات ذكية، ويتم تجاوز أوقات الحظر الإجبارية تلقائياً.</p>
...
if __name__ == '__main__':
    # الحصول على رقم البورت من بيئة التشغيل لضمان عمل Render بشكل صحيح
