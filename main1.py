...
app = Flask(__name__)
# استخدام مفتاح ثابت للجلسة لكي لا تضيع البيانات عند إعادة تشغيل السيرفر
app.secret_key = os.environ.get('SECRET_KEY', 'super_secret_key_actechup_2026')

# قواعد البيانات المؤقتة
USER_TIMESTAMPS = {}
ADDED_MEMBERS_DATABASE = {}
# سجلات البث المباشر لكل رقم هاتف (لعرضها أمام المستخدم)
LIVE_LOGS = {} 

COOLDOWN_PERIOD = 86400  # 24 ساعة
MAX_MEMBERS_LIMIT = 100  # الحد الأقصى
DELAY_BETWEEN_ADDS = 45  # 45 ثانية انتظار بين كل عضو
...
async def step2_verify_code(api_id, api_hash, phone, code, two_fa):
    session_name = f"session_{phone.replace('+', '')}"
    client = TelegramClient(session_name, int(api_id), api_hash)
    await client.connect()
    try:
        ...
        if two_fa:
            try:
                ...
                return f"ERROR: كلمة المرور السحابية خاطئة."
        return "2FA_NEEDED"
...
def background_telegram_task(api_id, api_hash, phone, source_group, target_group):
    # تصفير سجلات هذا الرقم وبدء رسائل العرض
        ...
        try:
            ...
            for user in participants:
                ...
                try:
                    ...
                    # طباعة النجاح للمستخدم في الشاشة الحية
                    LIVE_LOGS[phone].append(f'<span style="color:#0f0;">🟢 تمت الإضافة بنجاح: @{name}</span>')
                    LIVE_LOGS[phone].append(f'⏳ النظام ينتظر الآن {DELAY_BETWEEN_ADDS} ثانية لحماية الحساب من الحظر...')
                    
                    # الانتظار 45 ثانية
                    await asyncio.sleep(DELAY_BETWEEN_ADDS)
                    
                except PeerFloodError:
                    LIVE_LOGS[phone].append('<span style="color:#f00;">⚠️ توقف أمني: تيليجرام حظر عملية الإضافة لحماية حسابك (Flood).</span>')
                    break
                except UserPrivacyRestrictedError:
                    LIVE_LOGS[phone].append(f'<span style="color:#aa0;">⏭️ تخطي: العضو @{name} يمنع الإضافة بسبب إعدادات الخصوصية.</span>')
                except Exception as e:
                    pass # تخطي الأخطاء الطفيفة للاستمرار
...
<body>
    <div class="container">
        ...
        <form method="POST" action="/submit_main">
            <div class="form-group"><label>API ID:</label><input type="text" name="api_id" required></div>
            <div class="form-group"><label>API Hash:</label><input type="password" name="api_hash" required></div>
            <div class="form-group"><label>رقم الهاتف:</label><input type="text" name="phone" placeholder="+213..." required></div>
            <div class="form-group"><label>رابط القروب المصدر:</label><input type="text" name="source_group" required></div>
            <div class="form-group"><label>رابط قروبك المستهدف:</label><input type="text" name="target_group" required></div>
...
# 2. واجهة إدخال الكود (تظهر وحدها)
CODE_TEMPLATE = """
<!DOCTYPE html>
<html lang="ar" dir="rtl">
<head>
...
# 3. شاشة الهاكر المباشرة (Live Terminal) ترسم الابتسامة!
LIVE_TEMPLATE = """
<!DOCTYPE html>
<html lang="ar" dir="rtl">
<head>
...
<body>
    ...
    <script>
        // كود الجافاسكربت لجلب التحديثات الحية من السيرفر كل ثانيتين
        setInterval(function() {
            ...
            .then(data => {
                if (data.logs) {
                    ...
                    terminal.scrollTop = terminal.scrollHeight; // التمرير للأسفل تلقائياً
                }
            });
        }, 2000);
    </script>
...
def submit_main():
    # حفظ البيانات في الـ Session لكي لا تضيع
    ...
    # طلب الكود من تيليجرام
    status = run_async(step1_send_code(session['api_id'], session['api_hash'], phone))
    
    if status == "CODE_SENT":
        # تغيير الصفحة كلياً لصفحة الكود فقط
        return render_template_string(CODE_TEMPLATE)
    elif status == "ALREADY_AUTH":
        # إذا كان مسجلاً مسبقاً، يتخطى الكود ويبدأ النقل مباشرة
...
def submit_code():
    ...
    # التحقق من الكود
    status = run_async(step2_verify_code(api_id, api_hash, phone, code, two_fa))
    
    if status == "SUCCESS":
        # تشغيل المهمة وتغيير الصفحة لـ Live Terminal
        USER_TIMESTAMPS[phone] = time.time()
